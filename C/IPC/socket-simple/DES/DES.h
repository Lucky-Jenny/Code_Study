...
/*
void Test_Print(char *format, ...)	// This format will print wrong message.
{
	va_list arglist;
#if TEST
...
static bool SubKey[16][48]={0};		// 16 wheel keys
...
// Turn Bit to Byte
void BitToByte(char My_message[STR_LEN],bool * Message_in,int Num)
{
	int i=0;
	for(i=0;i<(Num/8);i++){
		My_message[i]=0;
...
// Replacement Algoritm of Tables
...
{
	int i, j;
	static bool Key_bit[64]={0};		// buff of key in binary
	static bool *Key_bit_L, *Key_bit_R;
	Key_bit_L = &Key_bit[0];			// 28 bits left of key
	Key_bit_R = &Key_bit[28]; 		// 28 bits right of key
	ByteToBit(Key_bit, My_key, 64);
	/* Change_bit(Key_bit,64);
	for(i = 0; i < 64; i++){
		printf("%d ",Key_bit[i]);
	}
	printf("\n");
	...
	for(i = 0; i < 16; i++){
		...
		TableReplace(SubKey[i],Key_bit,PC2_Table,48);	//pc-2 Replacement
	}
}
...
{
	int i;
	static bool Message_E[48]={0};
	TableReplace(Message_E, Data_out, E_Table, 48);
	Xor(Message_E, Data_in, 48);
	S_change(Data_out, Message_E);
	TableReplace(Data_out, Data_out, P_Table, 32);
}

//DES Wheel Encryption Algorithm
void Run_Des(char My_message[],char HexMssage[])
{
	int i; 
	static bool Message_bit[64] = {0};
	static bool *Message_bit_L = &Message_bit[0];
	static bool *Message_bit_R = &Message_bit[32];
	static bool Temp[32] = {0};
	ByteToBit(Message_bit, My_message,64);
	/*Change_bit(Message_bit,64) ;
	for(i = 0; i < 64; i++){
...
// DES Wheel Decryption Algorithm
void Run_desDes(char My_message[],char HexMessage[])
{
	int i = 0;
	static bool Message_bit[64] = {0};
	static bool *Message_bit_L = &Message_bit[0];
