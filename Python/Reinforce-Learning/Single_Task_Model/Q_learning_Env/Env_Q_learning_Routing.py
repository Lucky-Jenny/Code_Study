...
class QLRoutingEnv:
        ...
        for t in range(self.time_limit):  # 时隙
            ...
            for s in range(self.T_len):
                ...
                if mode == 1:
                    ...
                    # 仿真
                    action = self.explore_in_stable(ps, q_table[s])
                    ps_new, state_new = self.action_route(task_space[s], action, router_space)
                    '记录路径'
    ...
    def explore_in_stable(self, now, q_array):
        nxt = np.argmax(q_array[now])
        require_search = False
        # 如果无法从Q表中获取有效值，
        if self.Action[now][nxt] == 0:
            require_search = True
        else:
            # 如果反向，检查是否陷入了死循环
            if nxt < now:
                nxt_nxt = np.argmax(q_array[nxt])
                if nxt_nxt == now:
                    require_search = True
        ...
        if require_search:
            ...
            '''
            for a in range(now, self.R_len, 1):
                if self.Action[now][a] == 1:
                    nxt = a
                    break
            '''
        return nxt
